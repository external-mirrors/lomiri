...
void LauncherModel::refresh()
{
    if (!m_accounts || m_user.isEmpty()) {
        ...
        QDBusPendingCall pendingCall = m_accounts->getUserPropertyAsync(m_user, QStringLiteral("com.ubports.lomiri.AccountsService"), QStringLiteral("LauncherItems"));
...
{
    if (user != m_user || interface != QLatin1String("com.ubports.lomiri.AccountsService") || !changed.contains(QStringLiteral("LauncherItems"))) {
        return;
    }
    refresh();
}
